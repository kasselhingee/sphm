...
SvMF estimation failed to find something good! But AIC is weirdly high.
...
Extreme AIC that could be for a multimodal distribution, but only one situation with known unimodal AIC. The AIC of this unimodal model is slightly better than the AIC of the model with only fstrike, Latitude, Longitude, Longitude.L148.

```{r}
unimodal_AIC <- lapply(restarts, function(mod){
  tryCatch({
    sphm:::SvMFcann(mod[["k"]], mod[["a"]], mod[["G0"]]) %>%
      sphm:::SvMFcann_check()
    mod$AIC
...
#| fig.width: 15
#| fig.height: 13
get_predobspairsdf(mod_SvMF, bind_cols(s4df_clean, rdist = mod_SvMF$dists)) %>%
  pairedplotggprep() +
...
The model predictions get very close to the antipode of G01. This is not good.
...
Doesnt seem to be as strong outliers as the model without distance and depth.
...
p1 <- bind_cols(rdist = mod_SvMF$dists, s4df_clean) %>%
  ...
  ggrepel::geom_label_repel(aes(label = Number),
                           size = 2,
                           data = ~filter(.x, rdist > 0.45), min.segment.length = 0.01) +
  ggrepel::geom_label_repel(aes(label = Number),
                           size = 2,
                           col = "blue",
