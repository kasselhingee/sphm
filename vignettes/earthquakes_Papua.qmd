...
ystd %>%
  ...
  ggplot(aes(x=A, y=B, z = fstrike)) +
  facet_grid(vars(pair2), vars(pair1), switch = "both") +
  geom_path(data = circle_df_long, mapping = aes(x=A,y=B), inherit.aes = FALSE, color = "grey") +
  ...
  stat_summary_2d(fun = mean, 
                  ...
                  bins = 10) +
  scale_fill_viridis_c(name = "fstrike") +
...
bind_cols(ystd, s4df) %>%
  ...
  select(starts_with("Y"), fstrike, outlier, Number, Longitude) %>%
  GGally::ggpairs(mapping = aes(col = fstrike, shape = Longitude > 148), columns = 1:5, upper = list(continuous = custom_points_wlabels), lower = NULL, diag = NULL) +
