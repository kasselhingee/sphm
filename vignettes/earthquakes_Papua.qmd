...
```{r}
pivot_coordpairs <- function(df, coordnames = paste0("Y",1:5)){
...
## Default start
```{r}
mod_vMF <- mobius_vMF(sphdf_clean %>% 
                        select(s1, s2, sMrt, sMrf, sMtf) %>% 
                        st_drop_geometry() %>% 
                        as.matrix(),
                      xs = NULL, 
...
# SvMF Regression
```{r}
mod_SvMF <- mobius_SvMF(sphdf_clean %>% 
                        select(s1, s2, sMrt, sMrf, sMtf) %>% 
                        st_drop_geometry() %>% 
                        as.matrix(),
                      xs = NULL, 
...
## Predictions
```{r}
get_predobspairsdf <- function(mod, extra = NULL){
  # standard rotations
...
#| fig.width: 15
#| fig.height: 13
get_predobspairsdf(mod_SvMF, sphdf_clean) %>%
  pairedplotggprep() +
...
## Paper Plot
```{r}
p1 <- get_predobspairsdf(mod_SvMF, sphdf_clean) %>%
  filter(pair %in% c("Y1-Y2", "Y2-Y3", "Y2-Y4")) %>%
  pairedplotggprep() +
  facet_wrap(vars(pair1, pair2), ncol = 3) +
  geom_point(aes(x=A, y=B, col = Longitude), size = 1) +
  scale_color_viridis_c()
p2 <- get_predobspairsdf(mod_SvMF, sphdf_clean) %>%
  filter(pair %in% c("Y1-Y2", "Y2-Y3", "Y2-Y4")) %>%
  pairedplotggprep() +
  facet_wrap(vars(pair1, pair2), ncol = 3) +
  geom_segment(aes(x=p_A, y=p_B, xend=A, yend=B, col = Longitude),
  ...
  geom_point(aes(x=A, y=B), size = 2, col = "blue",
             shape = 4,
             data = ~(attr(get_predobspairsdf(mod_SvMF), "p1") %>% filter(pair %in% .x$pair))
             ) +
  scale_color_viridis_c()
