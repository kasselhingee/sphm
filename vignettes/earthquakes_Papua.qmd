...
defaultplot_pred <- function(mod_SvMF, s4df_clean, focusaxes = FALSE, focusresponse = FALSE){
  ...
  if (!focusaxes){
    plotobj <- plotobj %>%
      addaxes(mod_SvMF, axiscolour = rep("grey", 4))
  }
  plotobj <- plotobj +
...
addaxes <- function(plotobj, mod_SvMF, axiscolour = c("red", "green", "blue", "purple")){
  plotobj +
...
bind_cols(mod_SvMF$rresids_G0, s4df_clean) %>%
  ...
  geom_point() +
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 1, col = "blue", data = ~filter(.x, covariate == "Depth"))
}
