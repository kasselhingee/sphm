...
calc_Paine_resids <- function(mod){
  mod$rresids <- rotatedresid(t(mod$Z), t(mod$predmean), base = north_pole(3))[,-1]
...
if (!is.null(svmfresult$G0)){
  ...
  mnaxes_proj <- (diag(3) - north_pole(3) %*% t(north_pole(3))) %*% attr(ystd, "std_rotation") %*% as_mnlink_cann(svmfresult$mean)$P
  eps = 0.2
  outplot <- outplot + 
