...
## SI Figure: Pairwise Projections of Earthquake Moment Tensors
```{r}
#| fig.width: 12
#| fig.height: 13
s4df %>%
...
mykbl <- df %>%
  ...
  kbl(df, booktabs = TRUE, position = "!h", escape = FALSE, format = "latex") %>%
...
mykbl <- df %>%
  ...
  kbl(df, booktabs = TRUE, position = "!h", escape = FALSE, format = "latex",
      row.names = FALSE,
      col.names = paste0("col", 1:ncol(df))) %>%
...
mykbl <- df %>%
  ...
  kbl(df, booktabs = TRUE, position = "!h", escape = FALSE, format = "latex",
      row.names = FALSE,
      col.names = paste0("col", 1:ncol(df))) %>%
  add_header_above(c("$R_e$" = ncol(df)), escape = FALSE)
kableExtra::save_kable(mykbl,
                       "earthq_Re.pdf", 
                       keep_tex = TRUE)
mykbl
...
mykbl <- df %>%
  ...
  mutate(across(everything(), as.character)) %>%
  kbl(df, booktabs = TRUE, position = "!h", escape = FALSE, format = "latex",
      col.names = c("$b_{01}$", paste0("col", 2:ncol(df)))) %>%
...
__should remove unfinished convergence results__
