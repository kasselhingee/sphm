...
The SvMF reduces to the vMF when all the scales `a` equal 1.
Under that null the axes `G0` are unidentifiable, so the usual chi-squared asymptotics for the likelihood ratio do not apply and we use a parametric bootstrap instead.

The Euclidean covariates below are taken from `mod$xe` rather than `xe`, so that the null and alternative models are fitted to exactly the same design matrix (`mod$xe` includes the dummy-zero and intercept columns added internally, and the standardisation used when `mod` was fitted).
...
This happens because under the null the axes `G0` are unidentifiable, so the SvMF likelihood is flat in those directions and the optimiser is slow to satisfy the tight `xtol_rel`.
