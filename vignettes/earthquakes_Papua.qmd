...
# Earthquake Moment Tensors Paired Plots
```{r}
#| fig.width: 15
#| fig.height: 13
s4df %>%
  left_join(outliers, by = "Number") %>%
  pivot_coordpairs(coordnames = c("s1", "s2", "sMrt", "sMrf", "sMtf")) %>%
  pairedplotggprep() +
  geom_point(aes(col = Longitude,
                 shape = is.na(outlier) | outlier == "depth" | outlier == "distance" |
           outlier == "fstrike")) +
...
# Earthquake Locations for Paper
```{r}
#| fig.width: 7
#| fig.height: 2
ggplot() +
...
# Paper Plot 2
```{r}
obsplots <- lapply(c("s1-s2", "sMtf-s2", "sMrt-s2"), function(pair){
...
predplots <- lapply(c("s1-s2", "sMtf-s2", "sMrt-s2"), function(pair){
