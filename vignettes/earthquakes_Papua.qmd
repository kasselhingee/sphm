...
s4df %>%
  ...
  mutate(outlier = case_when(Number == 93 ~ "maybeLongitude", TRUE ~ outlier)) %>%
  mutate(stdcoords = as_tibble(sphm:::standardise_sph(as.matrix(across(c(s1, s2, sMrt, sMrf, sMtf)))))) %>%
  tidyr::unnest_wider(stdcoords, names_sep = "") %>%
  rename_with(~gsub("stdcoordsV", "Y", .)) %>%
  pivot_coordpairs(coordnames = paste0("Y", 1:5)) %>%
...
s4df %>%
  ...
  geom_rug() +
  ggrepel::geom_label_repel(aes(y = 0, label = Number),
                            data = ~filter(.x, dist > 40000),
                            label.padding = 0.1,
                            min.segment.length = 0)
```
...
s4df %>%
  ...
  left_join(outliers, by = "Number") %>%
  filter(is.na(outlier)) %>%
  mutate(outlier = case_when(dist > 90000 ~ "bigdistance", TRUE ~ outlier)) %>%
  mutate(stdcoords = as_tibble(sphm:::standardise_sph(as.matrix(across(c(s1, s2, sMrt, sMrf, sMtf)))))) %>%
  tidyr::unnest_wider(stdcoords, names_sep = "") %>%
  rename_with(~gsub("stdcoordsV", "Y", .)) %>%
  ...
  pairedplotggprep() +
  stat_summary_2d(fun = mean,
                    aes(z = dist, fill = after_stat(value)),
                  bins = 10,
                  data = ~filter(.x, dist < 90000)) +
  scale_fill_viridis_c(name = "Distance") +
  geom_point(aes(fill = dist), size = 2, shape = 21,
             data = ~filter(.x, dist < 90000)) +
...
Consider two earthquakes with huge distance outliers:

```{r}
outliers <- c("92" = "distance",
  "93" = "distance") %>%
  tibble::enframe(name = "Number", value = "outlier") %>%
  mutate(Number = as.integer(Number)) %>%
  bind_rows(outliers)
...
s4df %>%
  ...
  left_join(outliers, by = "Number") %>%
  filter(is.na(outlier)) %>%
  mutate(outlier = case_when(fstrike < 1 ~ "lowfstrike", TRUE ~ outlier)) %>%
  mutate(stdcoords = as_tibble(sphm:::standardise_sph(as.matrix(across(c(s1, s2, sMrt, sMrf, sMtf)))))) %>%
  tidyr::unnest_wider(stdcoords, names_sep = "") %>%
  rename_with(~gsub("stdcoordsV", "Y", .)) %>%
  mutate(outlier = case_when(Y2 < -0.75 ~ "new", TRUE ~ outlier)) %>%
...
s4df %>%
  ...
  filter(is.na(outlier)) %>%
  ggplot() +
  geom_histogram(aes(x = fstrike), bins = 15) +
  geom_rug(aes(x = fstrike))
```

There could be two outliers with very low fault strike (54 and 305 - dark blue) relative to other earthquakes.

```{r}
outliers <- c("305" = "fstrike",
  "54" = "fstrike") %>%
  tibble::enframe(name = "Number", value = "outlier") %>%
  mutate(Number = as.integer(Number)) %>%
  bind_rows(outliers)
...
s4df %>%
  ...
  left_join(outliers, by = "Number") %>%
  filter(is.na(outlier)) %>%
  mutate(outlier = case_when(Depth == 19 ~ "depth", TRUE ~ outlier)) %>%
  mutate(stdcoords = as_tibble(sphm:::standardise_sph(as.matrix(across(c(s1, s2, sMrt, sMrf, sMtf)))))) %>%
  tidyr::unnest_wider(stdcoords, names_sep = "") %>%
  rename_with(~gsub("stdcoordsV", "Y", .)) %>%
  mutate(outlier = case_when(Y2 < -0.75 ~ "new", TRUE ~ outlier)) %>%
  ...
  pairedplotggprep() +
  stat_summary_2d(fun = mean,
                    aes(z = Depth, fill = after_stat(value)),
                  bins = 10) +
  scale_fill_viridis_c(name = "Depth") +
  geom_point(aes(fill = Depth), size = 2, shape = 21) +
  ggrepel::geom_label_repel(aes(label = Number),
                            data = ~filter(.x, !is.na(outlier)),
                            label.padding = 0.1,
                            min.segment.length = 0)
...
The two very deep earthquakes could also be outliers.

```{r}
outliers <- c("308" = "depth",
  "229" = "depth") %>%
  tibble::enframe(name = "Number", value = "outlier") %>%
  mutate(Number = as.integer(Number)) %>%
  bind_rows(outliers)
...
## Recheck Outlier Plots After Removal
```{r}
#| fig.width: 15
#| fig.height: 13
s4df %>%
...
s4df %>%
  ...
  left_join(outliers, by = "Number") %>%
  filter(is.na(outlier)) %>%
  mutate(outlier = case_when(dist > 35000 ~ "bigdistance", TRUE ~ outlier)) %>%
  mutate(stdcoords = as_tibble(sphm:::standardise_sph(as.matrix(across(c(s1, s2, sMrt, sMrf, sMtf)))))) %>%
  tidyr::unnest_wider(stdcoords, names_sep = "") %>%
  rename_with(~gsub("stdcoordsV", "Y", .)) %>%
  mutate(outlier = case_when(Y2 < -0.75 ~ "new", TRUE ~ outlier)) %>%
...
Possibly more outliers: Number 103.
...
s4df %>%
  ...
  left_join(outliers, by = "Number") %>%
  filter(is.na(outlier)) %>%
  mutate(outlier = case_when(fstrike < 1.25 ~ "fstrike", TRUE ~ outlier)) %>%
  mutate(stdcoords = as_tibble(sphm:::standardise_sph(as.matrix(across(c(s1, s2, sMrt, sMrf, sMtf)))))) %>%
  tidyr::unnest_wider(stdcoords, names_sep = "") %>%
  rename_with(~gsub("stdcoordsV", "Y", .)) %>%
  mutate(outlier = case_when(Y2 < -0.75 ~ "new", TRUE ~ outlier)) %>%
...
Maybe another 2 fstrike outliers above.
...
## Outlier Plots Again After Removal
```{r}
#| fig.width: 15
#| fig.height: 13
s4df %>%
...
s4df %>%
  ...
  left_join(outliers, by = "Number") %>%
  filter(is.na(outlier)) %>%
  mutate(outlier = case_when(dist > 30000 ~ "bigdistance", TRUE ~ outlier)) %>%
  mutate(stdcoords = as_tibble(sphm:::standardise_sph(as.matrix(across(c(s1, s2, sMrt, sMrf, sMtf)))))) %>%
  tidyr::unnest_wider(stdcoords, names_sep = "") %>%
  rename_with(~gsub("stdcoordsV", "Y", .)) %>%
  pivot_coordpairs(coordnames = paste0("Y", 1:5)) %>%
...
s4df %>%
  ...
  geom_rug() +
  ggrepel::geom_label_repel(aes(y = 0, label = Number),
                            data = ~filter(.x, dist > 30000),
                            label.padding = 0.1,
                            min.segment.length = 0)
```

Maybe 9 and 94 are outliers too.

```{r}
outliers <- c("9" = "distance",
  "94" = "distance") %>%
  tibble::enframe(name = "Number", value = "outlier") %>%
  mutate(Number = as.integer(Number)) %>%
  bind_rows(outliers)
...
xe <- s4df_clean %>%
  ...
  mutate(Longitude.L148 = L148*Longitude) %>%
  select(-L148) %>%
  as.matrix()
xestd <- xe %>%
  scale() |>
...
No real differences in estimates!
...
get_predobspairsdf <- function(mod,
  ...
  arrowends <- t(t(mod$G0[,-1] - mod$G0[,1]) * mod$a[-1]/20) + mod$G0[,1] # convert to difference, scale difference by scales a, add start back
  colnames(arrowends) <- paste0("G0", 1+ (1:ncol(arrowends)))
  
  axisarrows <- apply(mod$pred, 1, function(pred){
...
G0pairs <- get_predobspairsdf(mod_SvMF) %>%
  attr("G0")
orientations <- G0pairs %>%
  ...
  mutate(across(c(A, B), ~.x * scale/20)) %>% #scale axes by their estimated scale (G01 is 0 currently so scaling it does nothing)
  filter(Axis != "G01") %>%
  ungroup() %>%
...
#| fig.width: 15
#| fig.height: 13
get_predobspairsdf(mod_SvMF, bind_cols(s4df_clean, rdist = mod_SvMF$dists, rename_with(as_tibble(mod_SvMF$rresids_std), ~paste0("std", .x)))) %>%
  pairedplotggprep() +
  ...
  geom_segment(aes(x=Astart,y=Bstart, xend=A, yend=B),
               data = orientations %>% filter(Axis == "G02"),
               arrow = grid::arrow(length = unit(0.02, "npc"))) +
  geom_segment(aes(x=Astart,y=Bstart, xend=A, yend=B),
               col = "grey",
               data = orientations %>% filter(Axis == "G03"),
               arrow = grid::arrow(length = unit(0.02, "npc"))) +
  ...
  ggrepel::geom_label_repel(aes(label = Number),
                           ...
                           label.padding = 0.1,
                           min.segment.length = 0,
                           data = ~filter(.x, abs(stdr1) > 2 | abs(stdr2) > 2 | abs(stdr3) > 2 | abs(stdr4) > 2)) +
  scale_color_viridis_c() +
...
However the orientation axes via parallel transport vary a lot across the earthquakes, and not in an intuitive way because of the location of G01 - this makes interpretation really hard.
...
get_predobspairsdf(mod_SvMF, bind_cols(s4df_clean, rdist = mod_SvMF$dists, rename_with(as_tibble(mod_SvMF$rresids_std), ~paste0("std", .x)))) %>%
  ...
  geom_point(aes(x=p_A, y=p_B, col = Longitude), size = 2, shape = 1) +
  ggrepel::geom_label_repel(aes(x=A, y=B, label = Number),
  ...
  ggrepel::geom_label_repel(aes(label = Number),
                           ...
                           data = ~filter(.x, abs(stdr1) > 2 | abs(stdr2) > 2 | abs(stdr3) > 2 | abs(stdr4) > 2)) +
  scale_color_viridis_c() +
  ggtitle("Longitude: Response Highlighted")
```
...
get_predobspairsdf(mod_SvMF, bind_cols(s4df_clean, rdist = mod_SvMF$dists, rename_with(as_tibble(mod_SvMF$rresids_std), ~paste0("std", .x)))) %>%
  pairedplotggprep() +
  geom_segment(aes(x=p_A, y=p_B, xend=A, yend=B, col = fstrike),
               alpha = 0.5,
                arrow = grid::arrow(length = unit(0.02, "npc"))) +
  geom_point(aes(x=A, y=B, col = fstrike), size = 2) +
  geom_point(aes(x=p_A, y=p_B, col = fstrike), size = 2, shape = 1) +
  ggrepel::geom_label_repel(aes(x=A, y=B, label = Number),
                            data = function(x) {x %>% filter(!is.na(outlier))},
                            size = 2, 
                            label.padding = 0.1,
  ...
  ggrepel::geom_label_repel(aes(label = Number),
                           ...
                           data = ~filter(.x, abs(stdr1) > 2 | abs(stdr2) > 2 | abs(stdr3) > 2 | abs(stdr4) > 2)) +
  scale_color_viridis_c() +
  ggtitle("fstrike: Response Highlighted")
```
...
get_predobspairsdf(mod_SvMF, bind_cols(s4df_clean, rdist = mod_SvMF$dists, rename_with(as_tibble(mod_SvMF$rresids_std), ~paste0("std", .x)))) %>%
  pairedplotggprep() +
  geom_segment(aes(x=p_A, y=p_B, xend=A, yend=B, col = dist),
               alpha = 0.5,
                arrow = grid::arrow(length = unit(0.02, "npc"))) +
  geom_point(aes(x=A, y=B, col = dist), size = 2) +
  geom_point(aes(x=p_A, y=p_B, col = dist), size = 2, shape = 1) +
  ggrepel::geom_label_repel(aes(x=A, y=B, label = Number),
                            data = function(x) {x %>% filter(!is.na(outlier))},
                            size = 2, 
                            label.padding = 0.1,
  ...
  ggrepel::geom_label_repel(aes(label = Number),
                           ...
                           data = ~filter(.x, abs(stdr1) > 2 | abs(stdr2) > 2 | abs(stdr3) > 2 | abs(stdr4) > 2)) +
  scale_color_viridis_c() +
  ggtitle("dist: Response Highlighted")
```
...
get_predobspairsdf(mod_SvMF, bind_cols(s4df_clean, rdist = mod_SvMF$dists, rename_with(as_tibble(mod_SvMF$rresids_std), ~paste0("std", .x)))) %>%
  pairedplotggprep() +
  geom_segment(aes(x=p_A, y=p_B, xend=A, yend=B, col = Depth),
               alpha = 0.5,
                arrow = grid::arrow(length = unit(0.02, "npc"))) +
  geom_point(aes(x=A, y=B, col = Depth), size = 2) +
  geom_point(aes(x=p_A, y=p_B, col = Depth), size = 2, shape = 1) +
  ggrepel::geom_label_repel(aes(x=A, y=B, label = Number),
                            data = function(x) {x %>% filter(!is.na(outlier))},
                            size = 2, 
                            label.padding = 0.1,
  ...
  ggrepel::geom_label_repel(aes(label = Number),
                           ...
                           data = ~filter(.x, abs(stdr1) > 2 | abs(stdr2) > 2 | abs(stdr3) > 2 | abs(stdr4) > 2)) +
  scale_color_viridis_c() +
  ggtitle("Depth: Response Highlighted")
```

The three large residual earthquakes (108, 299, 217) dont seem to have difference in moment tensors that are related obviously to the covariates.

The three unusual residual earthquakes (89, 187, 209)
...
mod_SvMF$rresids_G0 %>%
  ...
  ggrepel::geom_label_repel(aes(label = Number),
                           size = 2,
                           col = "blue",
                           data = ~filter(.x, abs(stdr1) > 2 | abs(stdr2) > 2 | abs(stdr3) > 2 | abs(stdr4) > 2), min.segment.length = 0.01) +
  ggrepel::geom_label_repel(aes(x=A, y=B, label = Number),
                            data = ~filter(.x, rdist > 0.5),
                            size = 2, 
                            col = "black",
...
Three earthquakes with very large residuals after standardisation: 89, 187, 209.

```{r}
outliers <- c("89" = "20250723modelresidual",
  "187" = "20250723modelresidual",
  "209" = "20250723modelresidual") %>%
  tibble::enframe(name = "Number", value = "outlier") %>%
  mutate(Number = as.integer(Number)) %>%
  bind_rows(outliers) %>%
  distinct()
s4df_clean <- left_join(s4df_clean, outliers, by = "Number") %>%
  mutate(outlier = case_when(!is.na(outlier.x) ~ outlier.x,
                             !is.na(outlier.y) ~ outlier.y,
                             TRUE ~ outlier.x))
```
...
p1 <- bind_cols(rdist = mod_SvMF$dists, s4df_clean) %>%
  ...
  ggrepel::geom_label_repel(aes(label = Number),
                           size = 2,
                           data = ~filter(.x, rdist > 0.5), min.segment.length = 0.01) +
  ggrepel::geom_label_repel(aes(label = Number),
                           size = 2,
                           col = "blue",
                           data = ~filter(.x, !is.na(outlier)), min.segment.length = 0.01)
...
## Scaled Residual Distance by Covariate
```{r}
...
p1 <- bind_cols(srdist = stdrdist, rdist = mod_SvMF$dists, s4df_clean) %>%
  ggplot(aes(x=srdist, y = Longitude)) +
  geom_point() +
  ggrepel::geom_label_repel(aes(label = Number),
                           size = 2,
                           data = ~filter(.x, rdist > 0.5), min.segment.length = 0.01) +
...
These plots show very little association between longitude or fstrike and scaled residual distance.
...
bind_cols(mod_SvMF$rresids_G0, s4df_clean) %>%
  ...
  tidyr::pivot_longer(c(Longitude, fstrike, dist), names_to = "covariate", values_to = "cvalue") %>%
