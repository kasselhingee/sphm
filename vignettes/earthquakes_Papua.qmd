...
get_predobspairsdf <- function(mod,
  ...
  # apply to G01 too
  G01std <- standardise_sph(matrix(mod$G0[,1], nrow = 1), attr(ystd, "std_rotation"))
  colnames(G01std) <- paste0("Y", 1:ncol(ystd))
  G01pairs <- as_tibble(G01std) %>%
    pivot_coordpairs()
...
get_predobspairsdf(mod_SvMF, bind_cols(s4df_clean, rdist = mod_SvMF$dists, rename_with(as_tibble(mod_SvMF$rresids_std), ~paste0("std", .x)))) %>%
  ...
  geom_point(aes(x=A, y=B), size = 2, col = "red",
             data = attr(get_predobspairsdf(mod_SvMF), "G01")) +
