...
prelim_R_obj <- function(theta, y, x){
  prelimobj(y, x, Omega_proj(mnlink_Omega_unvec(theta, ncol(y), check = FALSE)))
}
prelim_R_Omegaconstraints <- function(theta, y, x){
...
prelim_R <- function(y, x, paramobj0, global = TRUE, local = TRUE){ #paramobj0 is the starting parameter object
  p <- ncol(y)
  ...
  if (global){
    globopt <- nloptr::nloptr(
      ...
      eval_f = prelim_R_obj,
      eval_g_eq = prelim_R_Omegaconstraints,
