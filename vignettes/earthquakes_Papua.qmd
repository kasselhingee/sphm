...
xe <- s4df_clean %>%
  st_drop_geometry() %>%
  mutate(val = 1) %>%
  tidyr::pivot_wider(names_from = Depth, values_from = val, values_fill = 0, names_prefix = "Depth") %>%
  select(fstrike,
         Latitude, 
         Longitude,
         dist,
         starts_with("Depth"), -Depth3) %>%
...
## Conclusion
Model looks okay. Optimisation hard.
AIC only slightly better than strike + longitude + L148.Longitude model.
