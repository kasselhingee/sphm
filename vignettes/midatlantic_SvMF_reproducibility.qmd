...
predplot <- function(svmfresult){
  ...
  # mark denominator in spherical covariate link
  denomloc <- drop(attr(ystd, "std_rotation") %*% sphm:::as_mnlink_cann(svmfresult$mean)$Qs[,1])
  outplot <- outplot +
    annotate("point", x = denomloc[2], y = denomloc[3], shape = 3, size = 4)
  
  # mark mean of spherical covariate (obtained earlier)
  meanXloc <-  drop(attr(ystd, "std_rotation") %*% xs)
  outplot <- outplot +
    annotate("point", x = meanXloc[2], y = meanXloc[3], shape = 24, size = 2, col = "grey")
...
plotours + theme(legend.position = "none") + ggtitle("Mobius SvMF") +
  ...
  plotESAG2 + ggtitle("Paine et al Structure 2") +
