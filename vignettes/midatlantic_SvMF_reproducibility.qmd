...
predplot <- function(svmfresult){
  ...
  outplot <- outplot +
    annotate("point", x = denomloc[2], y = denomloc[3], shape = 3, size = 2, col = "blue")
...
#| fig.width: 12
#| fig.height: 3.5
(plotdata + ggtitle("Data") + theme(plot.title = element_text(hjust = 0.5))) +
(plotours + theme(legend.position = "none", plot.title = element_text(hjust = 0.5)) + ggtitle("Ours")) +
  (plotESAG1 + ggtitle("Structure 1") + theme(plot.title = element_text(hjust = 0.5))) +
  (plotESAG2 + ggtitle("Structure 2") + theme(plot.title = element_text(hjust = 0.5))) +
...
```{r}
mod2 <- sphm:::optim_constV(y = fulldf %>% select(starts_with("Y")) %>% as.matrix(),
       xs = fulldf %>% select(starts_with("X")) %>% as.matrix(),
       xe = xestd %>% as.matrix(),
       G01behaviour = "free",
       mean = mod$mean,
