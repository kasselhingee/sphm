...
list(Y = fulldf %>% select(starts_with("Y")) %>% as.matrix(),
     Xs = fulldf %>% select(starts_with("X")) %>% as.matrix(),
     Xe = xestd %>% as.matrix()) |>
saveRDS("midatlantic_regression_data.rds")
saveRDS(mod, "midatlantic_model.rds")
```
...
mod_prop4i <- mobius_SvMF(
  y = y_data, xs = xs_data, xe = NULL,
  type = "Prop4i", det_constraint = "orthogonal", G01behaviour = "free")

# started from the spherical-only fit
mod_prop4i_all <- mobius_SvMF(
  y = y_data, xs = xs_data, xe = xe_raw, mean = mod_prop4i$mean,
  type = "Prop4i", intercept = TRUE,
  det_constraint = "orthogonal", G01behaviour = "free")

mod_prop4ii <- mobius_SvMF(
  y = y_data, xs = xs_data, xe = NULL,
  type = "Prop4ii", det_constraint = "orthogonal", G01behaviour = "free")

mod_prop4ii_all <- mobius_SvMF(
  y = y_data, xs = xs_data, xe = xe_raw,
  type = "Prop4ii", intercept = TRUE,
  det_constraint = "orthogonal", G01behaviour = "free")

prop4mods <- list(
  `Proposition 4(i): spherical only`             = mod_prop4i,
  `Proposition 4(i): spherical + all Euclidean`  = mod_prop4i_all,
  `Proposition 4(ii): spherical only`            = mod_prop4ii,
  `Proposition 4(ii): spherical + all Euclidean` = mod_prop4ii_all
)
```
...
predplot <- function(svmfresult){
  ...
  xs <- xs/vnorm(xs)
  eastwestedge <- mobius_link(xs = rbind(xs, xs),
         xe = tmpxe,
  ...
  # rs1 <- as_mobius_link_cann(svmfresult$mean)$Qs[,1]
  ...
  prettyQs <- attr(ystd, "std_rotation") %*% as_mobius_link_cann(svmfresult$mean)$Qs
  outplot <- outplot +
...
if (!is.null(svmfresult$G0)){
  ...
  mnaxes_proj <- (diag(3) - north_pole(3) %*% t(north_pole(3))) %*% attr(ystd, "std_rotation") %*% as_mobius_link_cann(svmfresult$mean)$P
  eps = 0.2
  outplot <- outplot + 
...
IAG1 <- R.matlab::readMat("SEuc2S_IAG1.mat")
...
ESAG1 <- R.matlab::readMat("SEuc2S_ESAG1.mat")
ESAG1$Lik <- 251.7973
...
ESAG2 <- R.matlab::readMat("SEuc2S_ESAG2.mat")
ESAG2$Lik <- 243.1426
...
c(Ours = mod$DoF, IAG1 = IAG1$DoF, ESAG1 = ESAG1$DoF, ESAG2 = ESAG2$DoF,
  vapply(prop4mods, function(m) m$DoF, numeric(1)))
```
...
c(Ours = mod$AIC, IAG1 = IAG1$AIC, ESAG1 = ESAG1$AIC, ESAG2 = ESAG2$AIC,
  vapply(prop4mods, function(m) m$AIC, numeric(1)))
```
