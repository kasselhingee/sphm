...
restarts <- pbapply::pblapply(1:100, function(seed){
  ...
  start <- rmnlink_cann(p = 5, qs = 0, qe = ncol(xestd) + 2, preseed = seed)
...
```{r}
cann <- as_mnlink_cann(mod_SvMF$mean)
```
...
get_predobspairsdf <- function(mod,
  ...
  axisarrows <- lapply(1:nrow(mod$pred), function(idx){
    ...
    stdarrowends <- attr(ystd, "std_rotation") %*% rotationmat_amaral(mod$G0[,1], pred) %*% arrowends
...
mod_vMF <- mobius_vMF(y = mod_SvMF$y,
                     ...
                     xe = mod_SvMF$xe,
                     type = "Shogo")
mod_vMF_restarts <- pbapply::pblapply(1:100, function(seed){mobius_vMF_restart(mod_vMF, seed)})
...
getlikR <- function(y){
  ...
  lLik1 <- ldMobius_SvMF(mod1$y, xs = NULL, xe =mod1$xe,
              mean = mod1$est,
              k = mod1$k,
              a = rep(1, 5),
              G0 = mod2$G0) %>%
    colSums()
  lLik2 <- ldMobius_SvMF(mod2$y, xs = NULL, xe =mod2$xe,
              mean = mod2$mean,
              k = mod2$k,
              a = mod2$a,
              G0 = mod2$G0) %>%
