...
# Look for Outliers
Here I'll look for obvious outliers using Longitude and fstrike and distance.
...
+ Some outliers in standard residuals
+ Distance appears to be well accounted for.
...
+ Optimisation difficutly

This model good except for the optimisation difficulty.
...
fail <- vapply(restarts, inherits, "try-error", FUN.VALUE = TRUE)
fails <- restarts[fail]
restarts <- restarts[!fail]
lapply(restarts, "[[", "AIC") %>%
  unlist() %>%
  tibble::enframe("seed", "AIC") %>%
  ggplot()+
...
+ Some outliers in standardised residuals
+ Some suggestion of increasing `srdist` with longitude.
