...
Maybe 175, 87 and 169 are outliers in Y1, but seems a bit questionable.
...
Maybe the earthquake 229 is an outlier.

```{r}
outliers <- c("229" = "distance") %>%
  tibble::enframe(name = "Number", value = "outlier") %>%
  mutate(Number = as.integer(Number)) %>%
  bind_rows(outliers)
```
...
s4df_clean <- s4df %>%
  left_join(outliers, by = "Number") %>%
  filter(is.na(outlier) | outlier != "general")
