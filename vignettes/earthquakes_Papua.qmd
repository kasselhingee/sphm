...
## Conclusion
Model looks good. AIC is on par with having fstrike, dist and Depth categorical, and better than just fstrike.

# Simulation of Unscaled Residual Distance
## Residual Distance by Covariate (Unstandardised)
```{r}
...
## Simulate Residuals
```{r}
simdists <- pbapply::pblapply(1:100, function(seed){
  set.seed(seed)
  y <- sphm:::rS2S_constV(xs = NULL, 
                   ...
                   a = mod_SvMF$a,
                   G0 = mod_SvMF$G0)[,-6]
  simdists <- acos(rowSums(mod_SvMF$pred * y))
  bind_cols(rdist = simdists, s4df_clean)
}) %>%
  bind_rows()
p1 <- simdists %>%
  ggplot(aes(y=rdist, x = Longitude)) +
  # geom_point() +
  geom_violin(aes(group = Longitude), width = 0.1) +
  geom_quantile(quantiles = c(0.1, 0.5, 0.9))

p2 <- simdists %>%
  ggplot(aes(y=rdist, x = fstrike)) +
  # geom_point() +
  geom_violin(aes(group = fstrike), width = 0.02) +
  geom_quantile(quantiles = c(0.1, 0.5, 0.9))
p3 <- simdists %>%
  group_by(Longitude) %>%
  summarise(r90 = quantile(rdist, 0.90)) %>%
  ggplot(aes(x = Longitude, y = r90)) +
  geom_point()
p4 <- simdists %>%
  group_by(fstrike) %>%
  summarise(r90 = quantile(rdist, 0.90)) %>%
  ggplot(aes(x = fstrike, y = r90)) +
  geom_point()
...
Simulations are performing as I expected - no trend in residual size.
...
+ Optimisation hard
+ Antipode of p1 obtained - at L < 148.

Model badish.

AIC slightly worse than equivalent with fstrike.
...
## Conclusion
+ AIC worse than equivalent model with fstrike
+ Obtains G01 antipode singularity
...
#| fig.height: 13
circle_df_long <- circle_df_long %>%
  mutate(across(starts_with("pair"), ~gsub("r", "Y", .x)))
