...
rownames(raw$X) <- paste0("X", 1:3)
rownames(raw$Y) <- paste0("Y", 1:3) #Y3 runs from north pole towards south pole
fulldf <- bind_cols(t(raw$X), t(raw$Y)) %>%
  mutate(westedge = X2 < -0.3)
```
...
plotdata <- bind_cols(prettyY, prettyX) %>%
  as_tibble() %>%
  ggplot() +
  geom_segment(aes(x=X2, y=X3, xend=Y2, yend=Y3), lty = "dashed", col = "grey") +
  geom_point(aes(x=Y2, y=Y3)) +
  geom_point(aes(x=X2, y=X3), shape = 4) +
  geom_path(data = circle_df, aes(x = x, y = y), inherit.aes = FALSE, color = "grey") +
  theme_void() +
  coord_fixed()
plotdata
...
calc_Paine_resids <- function(mod){
  mod$rresids <- rotatedresid(t(mod$Ystd), t(mod$predmean), base = sphm:::nthpole(3))[,-1]
  mod$dists <- acos(rowSums(t(mod$Ystd) * t(mod$predmean)))
  colnames(mod$rresids) <- c("r1", "r2")
  return(mod)
}

predplot_ESAG <- function(esagmod){
  stdmat <- standardise_mat(t(esagmod$Ystd))
  stdmat[,3] <- -stdmat[,3]
  #get back to Earth coordinates
  rot1 <- standardise_mat(fulldf %>% select(starts_with("Y")) %>% as.matrix())
  rot <- prettyrot %*% rot1 %*% stdmat
  ystd <- t(rot %*% esagmod$Ystd)
  nthproj <- (diag(3) - sphm:::nthpole(3) %*% t(sphm:::nthpole(3)))
  colnames(ystd) <- paste0("Ys", 1:3)
  obspred <- bind_cols(obs = as_tibble((ystd %*% nthproj)[,-1]),
            pred = as_tibble((t(nthproj %*% rot %*% esagmod$predmean))[,-1]) %>% rename_with(.fn = ~ paste0("p", .x)),
            .name_repair = "unique")
  obspred %>%
    as_tibble() %>%
    ggplot() +
...
Note that correlation between X1X3 and X3 is very high, but excluding X1X3 reduces the AIC of the below model to -460 (about 75 units worse). We are able to do slightly better than the Paine et al regressions here because the Paine et al methods are not improved as much by the inclusion of X1X3.
...
#| include: false
mySvMF <- function(xe){
  modvMF_default <- mobius_vMF(y = fulldf %>% select(starts_with("X")) %>% as.matrix(),
      xs = fulldf %>% select(starts_with("Y")) %>% as.matrix(),
      xe = xe,
      fix_qs1 = FALSE, type = "Shogo")
  start <- modvMF_default$est
  ymean <- colMeans(fulldf %>% select(starts_with("X")) %>% as.matrix())
  ymean <- ymean/sqrt(sum(ymean^2))
  modSvMF <- mobius_SvMF(y = fulldf %>% select(starts_with("X")) %>% as.matrix(),
       xs = fulldf %>% select(starts_with("Y")) %>% as.matrix(),
       xe = xe,
       mean = start,
       G01behaviour = "free",
...
```{r ours}
#| include: false
mod <- mySvMF(xe = xestd %>% dplyr::select(westedge, Y1, Y3, Y1Y3) %>% as.matrix())
mod[c("k", "DoF", "AIC")]
```
...
predplot <- function(svmfresult){
  ...
  xs <- xetrans$`scaled:center`[c("Y1", "Y2", "Y3")]/sphm:::vnorm(xetrans$`scaled:center`[c("Y1", "Y2", "Y3")]) #xs is mean of Y
  xe_nat <- c(xs, westedge = 0, Y1Y3 = unname(xs[1] * xs[3]))
  ...
  # outplot <- outplot +
  #   annotate("point", x = eastwestedge[1, 2], y = eastwestedge[1, 3], col = "green", shape = "o", size = 2) +
  #   annotate("point", x = eastwestedge[2, 2], y = eastwestedge[2, 3], col = "red", shape = "x", size = 2)
...
if (!is.null(svmfresult$G0)){
  ...
  annotate("segment", x = G0proj[2,1], y = G0proj[3,1],
           ...
           yend = G0proj[3,1] + eps*G0proj[3,2], col = "blue",
           arrow = grid::arrow(length = unit(0.01, "npc"))) +
  annotate("segment", x = G0proj[2,1], y = G0proj[3,1],
           xend = G0proj[2,1] + eps*G0proj[2,3], 
           yend = G0proj[3,1] + eps*G0proj[3,3], col = "green",
           arrow = grid::arrow(length = unit(0.01, "npc"))) +
  ...
  annotate("segment", x = mnaxes_proj[2,1], y = mnaxes_proj[3,1],
           ...
           yend = mnaxes_proj[3,1] + eps*mnaxes_proj[3,3], col = "grey",
           arrow = grid::arrow(length = unit(0.01, "npc"))) 
}
return(outplot)
}
...
```{r iag1}
#| include: false
IAG1 <- R.matlab::readMat("../../sphere_reg_code/westedge_X1X3/SEuc2S_IAG1.mat")
IAG1 <- calc_Paine_resids(IAG1)
IAG1$DoF <- (3*nrow(IAG1$Cov) + 1 + 1)
IAG1$AIC <- 2*IAG1$DoF - 2 * (-IAG1$fval)
IAG1$AIC
...
```{r esag1}
#| include: false
ESAG1 <- R.matlab::readMat("../../sphere_reg_code/westedge_X1X3/SEuc2S_ESAG1.mat")
ESAG1 <- calc_Paine_resids(ESAG1)
ESAG1$DoF <- (4*nrow(ESAG1$Cov) + 1 + 1)
ESAG1$AIC <- 2*ESAG1$DoF - 2 * (-ESAG1$fval)
ESAG1$AIC
...
```{r esag2}
#| include: false
ESAG2 <- R.matlab::readMat("../../sphere_reg_code/westedge_X1X3/SEuc2S_ESAG2.mat")
ESAG2$predmean <- ESAG2$mean.SEuc2S.esag
ESAG2 <- calc_Paine_resids(ESAG2)
...
```{r}
#| echo: false
c(Ours = mod$DoF, IAG1 = IAG1$DoF, ESAG1 = ESAG1$DoF)
```

# AIC
```{r}
#| echo: false
c(Ours = mod$AIC, IAG1 = IAG1$AIC, ESAG1 = ESAG1$AIC)
```
...
#| fig.width: 12
#| fig.height: 4
plotdata + ggtitle("Data") +
plotours + theme(legend.position = "none") + ggtitle("Mobius SvMF") +
  plotESAG1 + ggtitle("Paine et al Structure 1") + 
  ((d1plot+theme(legend.position = "none"))/d2plot +
  plot_layout(axes = "collect", heights = c(3,1))) +
  plot_layout(ncol = 4, widths = c(3,3,3,2))
```

Caption: 
From left: Midatlanic ridge and corresponding locations on the continent from Rosenthal el at (2014);
Regression using our mobius link and the scaled von mises fisher; regression using Paine et al Structure 1;
geodesic size of residuals.
The sphere is shown orthogonally projected with north pointing up the page.
Arrows start at the predicted mean and end at the observed midatlantic ridge location.
Blue/dashed: our regression.
Red: Paine et al structure 1.
...
Since these are all positive and non-zero, which confirms that the optimisation routine has found a local maximum of the likelihood.
