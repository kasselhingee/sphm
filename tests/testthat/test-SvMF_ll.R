...
test_that("vMF normalising constant for p!=3 is the same from multiple methods", {
  ...
  k <- runif(1, 0, 100)
  p <- 15
  basec <- vMF_log_norm_const_exact(k, p, method = 'base')
  expect_equal(vMF_log_norm_const_exact(k, p, method = 'Bessel'), basec)
  expect_equal(vMF_log_norm_const_exact(k, p, method = 'movMF'), basec)
})
