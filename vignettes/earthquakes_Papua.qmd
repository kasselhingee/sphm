...
What does Longitude +/- Longitude2 look like?
Recalling these are standardised for the above scales.

```{r}
xe %>%
  as_tibble() %>%
  select(Longitude, Longitude2) %>%
  mutate(`L + L^2` = Longitude + Longitude2,
         `L - L^2` = Longitude - Longitude2) %>%
  ggplot() + 
  geom_point(aes(x = Longitude, y = Longitude2))
xe %>%
  as_tibble() %>%
  select(Longitude, Longitude2) %>%
  mutate(`L + L^2` = Longitude + Longitude2,
         `L - L^2` = Longitude - Longitude2) %>%
  ggplot() + 
  geom_point(aes(x = Longitude, y = -`L - L^2`))
xe %>%
  as_tibble() %>%
  select(Longitude, Longitude2) %>%
  mutate(`L + L^2` = Longitude + Longitude2,
...
How does Longitude2 vs Longitude look like a straight line!? At large values of 150ish, the square looks pretty straight?
...
Note that the earthquake with the biggest longitude has the smallest residual distance

```{r}
which.max(mod_SvMF$xe[, "Longitude"])
which.min(mod_SvMF$dists)
```
...
circle_df_long <- circle_df_long %>%
  mutate(across(starts_with("pair"), ~gsub("Y", "s", .x)))
...
#| fig.height: 7
circle_df_long <- circle_df_long %>%
  mutate(across(starts_with("pair"), ~gsub("s", "r", .x)))
mod_SvMF$rresids_G0 %>%
  as_tibble() %>%
...
MSE here was huge (0.18). Probably because the estimation is so unstable doing a full mobius_SvMF() call each time.
More stable is to only do the `optim_constV()` step. This reduced MSE to 0.17, which still isn't very good and is worse than the mean models below.
...
loocvmseRegionMeans <- function(mod){
  dists <- pbapply::pblapply(1:nrow(mod$y), function(idx){
    newmod <- regionalmeans(y = mod$y[-idx,, drop = FALSE],
                            ...
                            breaks = mod$breaks)
    # prediction
    region <- cut(Longitude[idx], breaks = newmod$breaks, dig.lab = 4)
