...
s4df_clean <- s4df %>%
  left_join(outliers, by = "Number") %>%
  filter(is.na(outlier) | outlier == "depth" | outlier == "distance" | outlier == "fstrike") %>%
...
#| fig.width: 15
#| fig.height: 13
get_predobspairsdf_nat(mainmod, s4df_clean %>% select(-starts_with("s"))) %>%
  pairedplotggprep() +
  geom_segment(aes(x=p_A, y=p_B, xend=A, yend=B, col = Longitude),
               alpha = 0.5,
                arrow = grid::arrow(length = unit(0.02, "npc"))) +
...
obsplots <- lapply(c("Y1-Y2", "Y3-Y2", "Y4-Y2"), function(pair){
  get_predobspairsdf(mod_SvMF, s4df_clean,
    ...
    pairedplotggprep() +
    geom_point(aes(x=A, y=B, col = Depth), size = 1) +
    scale_color_viridis_c() +
...
predplots <- lapply(c("Y1-Y2", "Y3-Y2", "Y4-Y2"), function(pair){
  ...
  geom_segment(aes(x=p_A, y=p_B, xend=A, yend=B, col = Depth),
               alpha = 0.5,
                arrow = grid::arrow(length = unit(0.02, "npc"))) +
  geom_point(aes(x=p_A, y=p_B, fill = Depth),
             size = 1.5,
             shape = 21) +
