...
#| include: false
mySvMF <- function(xe){
  modvMF_default <- mobius_vMF(y = fulldf %>% select(starts_with("Y")) %>% as.matrix(),
  ...
  ymean <- ymean/sqrt(sum(ymean^2))
  modSvMF <- mobius_SvMF(y = fulldf %>% select(starts_with("Y")) %>% as.matrix(),
       xs = fulldf %>% select(starts_with("X")) %>% as.matrix(),
       xe = xe,
       mean = start,
       G01behaviour = "free",
...
```{r ours}
#| include: false
mod <- mySvMF(xe = xestd %>% dplyr::select(westedge, X1, X3, X1X3) %>% as.matrix())
mod[c("k", "DoF", "AIC")]
```
...
predplot <- function(svmfresult){
  ...
  xs <- xetrans$`scaled:center`[c("X1", "X2", "X3")]/sphm:::vnorm(xetrans$`scaled:center`[c("X1", "X2", "X3")]) #xs is mean of X
  xe_nat <- c(xs, westedge = 0, X1X3 = unname(xs[1] * xs[3]))
