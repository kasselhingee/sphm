...
mod_vMF <- mobius_vMF(y = mod_SvMF$y,
                     ...
                     xe = mod_SvMF$xe,
                     type = "Shogo")
mod_vMF_restarts <- pbapply::pblapply(1:100, function(seed){sphm:::mobius_vMF_restart(mod_vMF, seed)})
...
getlikR <- function(y){
  ...
  lLik1 <- sphm:::ldMobius_SvMF(mod1$y, xs = NULL, xe =mod1$xe,
              mean = mod1$est,
              k = mod1$k,
              a = rep(1, 5),
              G0 = mod2$G0) %>%
    colSums()
  lLik2 <- sphm:::ldMobius_SvMF(mod2$y, xs = NULL, xe =mod2$xe,
              mean = mod2$mean,
              k = mod2$k,
              a = mod2$a,
              G0 = mod2$G0) %>%
...
null_likRs <- pbapply::pblapply(1:1000, function(seed){
  set.seed(seed)
  y_ld <- sphm:::rMobius_SvMF(xs = NULL, 
                   xe = mod_vMF$xe,
