...
target <- c(1,0,0)
prettyrot <- rotationmat_amaral(ymeanproj, target)
prettyY <- (fulldf %>%
  select(Y1, Y2, Y3) %>%
...
# denomloc <- drop(prettyrot %*% as_mnlink_cann(mod$mean)$Qs[,1])
...
calc_Paine_resids <- function(mod){
  mod$rresids <- rotatedresid(t(mod$Z), t(mod$predmean), base = nthpole(3))[,-1]
...
predplot_ESAG <- function(esagmod){
  nthproj <- (diag(3) - nthpole(3) %*% t(nthpole(3)))
...
predplot <- function(svmfresult){
  ...
  projmat <- (diag(3) - nthpole(3) %*% t(nthpole(3)))
  obs <- ystd %*% projmat[,-1]
  ...
  # rs1 <- as_mnlink_cann(svmfresult$mean)$Qs[,1]
  ...
  prettyQs <- attr(ystd, "std_rotation") %*% as_mnlink_cann(svmfresult$mean)$Qs
  outplot <- outplot +
...
if (!is.null(svmfresult$G0)){
  G0proj <- (diag(3) - nthpole(3) %*% t(nthpole(3))) %*% attr(ystd, "std_rotation") %*% svmfresult$G0
  mnaxes_proj <- (diag(3) - nthpole(3) %*% t(nthpole(3))) %*% attr(ystd, "std_rotation") %*% as_mnlink_cann(svmfresult$mean)$P
  eps = 0.2
  outplot <- outplot + 
...
Our mean link with free rs1 has `r mobius_DoF(p=dim(mod$mean)["p"],qs=dim(mod$mean)["qs"],qe=dim(mod$mean)["qe"], fix_qe1 = TRUE)` degrees of freedom.
...
restarts <- pbapply::pblapply(1:100, function(seed){
  start <- rmnlink_cann(p = 3, qs = 3, qe = ncol(xe) + 2, preseed = seed)
...
restarts <- pbapply::pblapply(1:100, function(seed){
  ...
  start <- rmnlink_cann(p = 3, qs = 3, qe = ncol(xestd) + 2, preseed = seed)
  # convert to LinEuc form:
  set.seed(seed+1)
...
```{r}
mod2 <- mobius_SvMF(y = fulldf %>% select(starts_with("Y")) %>% as.matrix(),
       xs = fulldf %>% select(starts_with("X")) %>% as.matrix(),
       xe = xestd %>% as.matrix(),
       G01behaviour = "free",
       mean = mod$mean,
       ...
       G0 = mod$G0,
       G0reference = mod$G0,
       fix_qs1 = FALSE, type = "LinEuc", doprelim = FALSE)
```
