...
mata1 meanlinkS2Scpp(const mata1 &xs, const mata1 &xe, const veca1 &vec, const int p, const int qe = 0) {
  ...
  if (ompar.qs > 0){
    ...
    ytilde = ytilde + numerator.rowwise()/denominator; //broadcast the denominator along each row
  }
  if (ompar.qe > 0){
  ...
  veca1 totdenom = 1.0 + ytildesizesq;
  mata1 numerator = p1 * (1.0 - ytildesizesq).transpose() + 2*ytilde;
